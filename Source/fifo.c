...
void fifo_copy(struct fifo_struct *target, struct fifo_struct *obj)
{
	unsigned int size_read, tempint=0, tempint2=0;
...
//	this next code is basically a copy of the read function, except instead of memcpy to a buffer, we call fifo_write to the target
...
char fifo_read_char(struct fifo_struct *obj)
{
	char retval=0;
...
int fifo_read_int(struct fifo_struct *obj)
{
	int retval;
...
long fifo_read_long(struct fifo_struct *obj)
{
	long retval;
...
char* fifo_read_string(struct fifo_struct *obj, char *target, int maxsize)
{
	char *retval;
	retval=target;
	if(maxsize)
...
void fifo_write_char(struct fifo_struct *obj, char data)
{
	if(obj!=NULL)
	{
		while(obj->full)
...
void fifo_write_int(struct fifo_struct *obj, int data)
{
	fifo_write(obj, &data, sizeof(int));
}

void fifo_write_long(struct fifo_struct *obj, long data)
{
...
void fifo_init(struct fifo_struct *obj, unsigned int size, void(*post_fptr)(void), void(*wait_fptr)(void), void *data_space)
{
	if(data_space == NULL)
	{
...
// read data from fifo
void* fifo_read(struct fifo_struct *obj, void *item, unsigned int size_target)
{
	unsigned int tempint=0, size_read=0;
...
void* fifo_preview(struct fifo_struct *obj, void *item, unsigned int size)
{
...
void fifo_write(struct fifo_struct *obj, void *item, unsigned int size_source)
{
	unsigned int tempint=0, size_write=0;
	unsigned char fragment=FALSE;
...
void fifo_flush(struct fifo_struct *obj)
{
	ATOMIC_BLOCK
	{
		obj->tail_ptr = obj->start;
